...
const EditProfile = () => {
    ...
    const [employeeDocuments, setEmployeeDocuments] = useState<DocumentsFormatTypes | null>(null)
    ...
    useEffect(() => {
        ...
        axiosInstance.get(`employeeDocuments/`).then(
                (res) => { setEmployeeDocuments(res.data[0])
    ...
    const handleDocumentsSubmit = async (values : DocumentsFormatTypes, { setSubmitting, setErrors ,setValues} : FormikHelpers<DocumentsFormatTypes>) => {
        ...
        try {
            ...
            if (!employeeDocuments) {
                response = await axiosInstance.post(`/employeeDocuments/`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            } else {
                response = await axiosInstance.patch(`/employeeDocuments/${employeeDocuments.id}/`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            }
            ...
            <div style={{ width: `50%`  }}>
                <div className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    {employeeDocuments && <Formik
                        initialValues={{                       
                            aadhar_card: employeeDocuments?.aadhar_card || '',
                            pan_card: employeeDocuments?.pan_card || '',
                            pan_image: employeeDocuments?.pan_image || '',
                            aadhar_image: employeeDocuments?.aadhar_image || ''
                        }}
