...
interface DocumentsFormatTypes{
        pan_card : string
        aadhar_card : string  
...
const UpdateEmployee = () => {
    ...
    const [basicDetails, setBasicDetails] = useState<EmployeeDataFormat | null>(null)
    // const [update,setUpdate] = useState(false)
    const [isFetching, setIsFetching] = useState<boolean>(true);
    const [employeeDocuments, setEmployeeDocuments] = useState<DocumentsFormatTypes []>([])
    const [employeeRelations, setEmployeeRelations] = useState<RelationFormatTypes []>([])
    ...
    console.log("this is my documents dATA",employeeDocuments.length>0 ? employeeDocuments[0]["aadhar_card"] : "")
    ...
    const handleSubmit = async (values : EmployeeDataFormat, { setSubmitting, setErrors,setValues } : FormikHelpers<EmployeeDataFormat>) => {
        try {
    ...
    const handleDocumentsSubmit = async (values : DocumentsFormatTypes, { setSubmitting, setErrors,setValues } : FormikHelpers<DocumentsFormatTypes>) => {
        ...
        try {
            ...
            if (employeeDocuments.length === 0) {
                response = await axiosInstance.post(`/employeeDocuments/?id=${id}`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            } else {
               
                response = await axiosInstance.patch(`/employeeDocuments/${employeeDocuments[0]["id"]}/`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            }
    ...
    const handleRelationsSubmit = async (values : RelationFormatTypes, { setSubmitting, setErrors ,setValues} : FormikHelpers<RelationFormatTypes>) => {
        console.log("this is all my values", values);
        try {
            let response;
            if (employeeRelations.length === 0) {
             response = await axiosInstance.post(`/companyRelations/?id=${id}`, values);             
              
            } else {
                console.log("THis is my finalllllll data",employeeRelations)
                // console.log("relations update", `/companyRelations/${employeeRelations[0]["id"]}/`);
              response = await axiosInstance.patch(`/companyRelations/${employeeRelations[0]["id"]}/`, values);
              
              
            }
            setEmployeeRelations([response.data])
            setValues({
                designation: response.data.designation,
                department: response.data.department,
                batch: response.data.batch ,
                joining_date: response.data.joining_date,
                probation_end_date : response.data.probation_end_date,
